...
Array: An array is a basic data structure used to store elements of the same data type contiguously in memory. Each element can be accessed directly using an index.Arrays provide O(1) random access to elements by index, but insertion and deletion operations in the middle are costly O(n) due to the need to shift elements.
